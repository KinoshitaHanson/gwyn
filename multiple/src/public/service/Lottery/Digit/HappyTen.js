...
class OneRed extends _HappyTen {
     ...
     */
    AwardCalc() {
        return this.JettonCalc(this.Selected) > 0 ? 5 : 0;
    }
    ...
    HighLightBetNumber(_selected, result) {
        ...
        return selected.map(m => {
            return {
                color: (result[0] == 19 || result[0] == 20) ? hightColor : '',
                value: m
            };
        });
    }
...
class TwoOptional extends _HappyTen {
    ...
    Random() {
        this.Selected = {
            first: super.Random([1, 20], 2, { zeroize: true }).sort((a, b) => a - b),
            second: [],
            third: []
        };
        return this.Selected;
    ...
    AwardCalc() {
        // 全选号码 最多中28注
        return this.JettonCalc() > 28 ? 28 * 8 : this.JettonCalc() * 8;
    }
...
class ThreeOptional extends _HappyTen {
    ...
    Random() {
        this.Selected = {
            first: super.Random([1, 20], 3, { zeroize: true }).sort((a, b) => a - b),
            second: [],
            third: []
        };
        return this.Selected;
     ...
     */
    AwardCalc() {
        return this.JettonCalc() > 56 ? 56 * 24 : this.JettonCalc() * 24;
    }
...
class FourOptional extends _HappyTen {
    ...
    Random() {
        this.Selected = {
            first: super.Random([1, 20], 4, { zeroize: true }).sort((a, b) => a - b),
            second: [],
            third: []
        };
        return this.Selected;
     ...
     */
    AwardCalc() {
        return this.JettonCalc() > 70 ? 70 * 80 : this.JettonCalc() * 80;
    }
...
class FiveOptional extends _HappyTen {
    ...
    Random() {
        this.Selected = {
            first: super.Random([1, 20], 5, { zeroize: true }).sort((a, b) => a - b),
            second: [],
            third: []
        };
        return this.Selected;
     ...
     */
    AwardCalc() {
        return this.JettonCalc() > 56 ? 56 * 320 : this.JettonCalc() * 320;
    }
...
class TwoContinuityCombination extends _HappyTen {
    ...
    AwardCalc() {
        ...
        return dic[this.JettonCalc()] || 217;
        // return this.JettonCalc() > 0 ? this.JettonCalc() * 31 : 0;
    }
    ...
    HighLightBetNumber(_selected, result) {
        ...
        result.forEach((m, i) => {
            ...
            if (selected.includes(m) && selected.includes(result[i + 1])) {
                award.push(m);
                award.push(result[i + 1]);
            }
        });

        return selected.map(m => {
            return {
                color: award.includes(m) ? hightColor : '',
                value: m
            };
        });
    }
...
class TwoContinuityDirect extends _HappyTen {
    ...
    HighLightBetNumber(_selected, result) {
        ...
        let i = result.findIndex(m => m == selected[0]);
        if (i > -1 && (result[i + 1] == selected[1])) {
            flag = true;
        }

        return selected.map(m => {
            return {
                color: flag ? hightColor : '',
                value: m
            };
        });
    }
...
class ThreeFrontCombination extends _HappyTen {
    ...
    HighLightBetNumber(_selected, result) {
        ...
        if (selected.includes(result[0]) && selected.includes(result[1]) && selected.includes(result[2])) {
            flag = true;
        ...
        return selected.map(m => {
            return {
                color: (flag && (m == result[0] || m == result[1] || m == result[2])) ? hightColor : '',
                value: m
            };
        });
    }
...
class ThreeFrontDirect extends _HappyTen {
    ...
    HighLightBetNumber(_selected, result) {
        ...
        return selected.map(m => {
            return {
                color: selected[0] == result[0] && selected[1] == result[1] && selected[2] == result[2] ? hightColor : '',
                value: m
            };
        });
    }
...
export class CQHappyTen {
    ...
    constructor(_type) {
        let key = Object.entries(CQHappyTen.PlayTypeEnum).find(m => m[1] == _type)[0];
        let type = CQHappyTen.PlayTypeEnum[key.replace('_Compound', '')];
