...
class Quick3 extends Digit {
    ...
    HighLightBetNumber(_selected, result) {
        ...
        let selected = this.Type == this.PlayTypeEnum.TwoSameSingle ? _selected : _selected.split(',');
        let FomulaMap = {
            ...
            [this.PlayTypeEnum.TwoSameSingle]: item => {
                ...
                if ((result[0] == result[1] || result[1] == result[2]) && (left.filter(m => m[0] == result[0]).length > 0 && right.includes(result[2].toString()))) {
                    let res = left.map(m => {
                        return {
                            color: hightColor,
                            value: m
                    ...
                    res.push({ color: hightColor, value: '#' });

                    let num = result.concat();
                    num.splice(1, 1);
                    num = num.join('');
                    res = res.concat(
                        right.map(m => {
                    ...
                    let res = left.map(m => {
                        return {
                            color: result[1] == m.substr(1, 1) && result[0] == m.substr(0, 1) && right.includes(result[3]) ? hightColor : '',
                            value: m
                        };
                    });
 ...
 * @extends {Quick3}
 */
export class JSQuick3 extends Quick3 {
    static PlayTypeEnum = {
        /**
         * 和值
         */
         ...
         * 三连号通选
         */
        ThreeContinuityAll: 6503,
        /**
         * 二同号复选
         */
        TwoSameCompound: 6506,
        /**
         * 二同号单选
         */
        TwoSameSingle: 6507,
         ...
         * 二不同号单选
         */
        TwoDiverseSingle: 6508
    };

    constructor(type) {
        super(JSQuick3.PlayTypeEnum, type);
    }
}
