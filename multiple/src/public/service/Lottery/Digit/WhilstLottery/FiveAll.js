...
export class FiveAll extends _WhilstLottery {
    ...
    HighLightBetNumber(selected, result) {
        ...
        let selectedArr = selected.split('').reverse();

        // 根据位置 数值 标红
        let coloredSelected = selectedArr.map((a, i) => ({
            value: a,
            color: a == reResult[i] ? hightColor : ''
        })).reverse();

        // 中奖标红，全部中奖
        if (coloredSelected.every(a => a.color) || (coloredSelected[0].color && coloredSelected[1].color) || (coloredSelected[3].color && coloredSelected[4].color)) {
            return coloredSelected;
        } else {
            return coloredSelected.map(a => ({
                value: a.value,
                color: ''
            }));
        }
