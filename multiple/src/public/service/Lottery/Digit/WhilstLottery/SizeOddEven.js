...
export class SizeOddEven extends _WhilstLottery {
    ...
    HighLightBetNumber(selected, result) {
        const hightColor = '#e23a3a',
            reResult = [...result].reverse().slice(0, 2), // 取个位 和 十位
            map = {
                1: '大',
                2: '小',
                3: '单',
        ...
        onesArr.push(reResult[0] === undefined || reResult[0] === '' ? 999 : reResult[0] % 2 == 0 ? 4 : 3);
        onesArr.push(reResult[0] === undefined || reResult[0] === '' ? 999 : reResult[0] < 5 ? 2 : 1);

        tensArr.push(reResult[1] === undefined || reResult[1] === '' ? 999 : reResult[1] % 2 == 0 ? 4 : 3);
        tensArr.push(reResult[1] === undefined || reResult[1] === '' ? 999 : reResult[1] < 5 ? 2 : 1);
        return selectedArr.map((a, i) => ({
            value: map[a],
            color: ([onesArr, tensArr][i]).some(b => a == b) ? hightColor : ''
        })).reverse();
    }
}
